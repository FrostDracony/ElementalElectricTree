...
namespace Creators
{
	...
	*/
	...
	class LODSetter : SRBehaviour
    {
		public void Start()
        {
			//GameContext.Instance.AutoSaveDirector.save
			Console.Log("LODSetter is working, yay");
			...
			{
				GetComponent<SlimeAppearanceApplicator>().Appearance = slimeAppearance.ShockedAppearance;
				GetComponent<SlimeAppearanceApplicator>().ApplyAppearance();
			}*/

			int lodInt = 0;

			foreach (LOD lod in GetComponent<SlimeAppearanceApplicator>().LODGroup.GetLODs())
			{
				lodInt++;
			...
			for (int j = 0; j < num; j++)
			{
				...
				Material goldSlimeMaterial = Instantiate(SRSingleton<GameContext>.Instance.SlimeDefinitions.GetSlimeByIdentifiableId(Identifiable.Id.GOLD_SLIME).AppearancesDefault[0].Structures[0].DefaultMaterials[0]);
				...
				if (component.TryGetComponent<MeshFilter>(out MeshFilter filter))
				{
					...
					component.GetComponent<MeshRenderer>().GetCopyOf(Main.assetBundle.LoadAsset<GameObject>("BoldCrest").GetComponentInChildren<MeshRenderer>(true));
					LOD[] lods = GetComponent<SlimeAppearanceApplicator>().LODGroup.GetLODs();

					lods[0].renderers[2] = Main.assetBundle.LoadAsset<GameObject>("BoldCrest").GetComponentInChildren<MeshRenderer>(true);
					lods[1].renderers[1] = Main.assetBundle.LoadAsset<GameObject>("BoldCrest").GetComponentInChildren<MeshRenderer>(true);
					GetComponent<SlimeAppearanceApplicator>().LODGroup.SetLODs(lods);
					break;
				}
				...
				slimeAppearance.Structures[1].DefaultMaterials[0] = goldSlimeMaterial;
